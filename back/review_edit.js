...
function init(){
	...
	$pfReview_form.addEventListener("mouseleave", function(){
		...
		if(document.getElementsByClassName('is_checked')){
			for($i=0; $i<Number($pfReview_form.attributes['star'].value); $i++) {
	...
	document.getElementById('icon-wrapper').addEventListener("mouseleave", function(){
		for($i=0; $i<$icon_user.length; $i++) {
			if(document.getElementById('icon-wrapper').attributes['sex'].value == $icon_user[$i].attributes['sex'].value){
...
function prev_checked(){
	remove_star_class('is_hover');
	document.getElementById('pfReview_form').attributes['star'].value = this.attributes['data-star'].value;
...
function icon_checked(){
	...
	document.getElementById('icon-wrapper').attributes['sex'].value = this.attributes['sex'].value;
}
...
<!-- 投稿ボタン押下時のチェック  -->
 function check(){
   ...
   <!-- アイコン  -->
   if(document.getElementById('icon-wrapper').attributes['sex'].value == ""){
		$error_comment.push("アイコンを選択してください\n");
		$check = false;
   }
   <!-- 評価  -->
   if(document.getElementById('pfReview_form').attributes['star'].value == "0"){
