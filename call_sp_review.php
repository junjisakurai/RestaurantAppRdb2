...
function exce_sp_review($proc, $menu_id, $user_id) {
    ...
    $dbh = connection_db();
	//DB取得
	$stmt = $dbh->prepare("CALL sp_review(?,?,?,null,null,null,null,null,null)");
...
function db_edit_sp_review($proc, $menu_id, $user_id, $user_m, $sex, $age, $review, $evaluation, $op_cd) {
    require_once('db_conn.php');
    $dbh = connection_db();
	//DB更新
